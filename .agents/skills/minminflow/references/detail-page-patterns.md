...
- Product drift: color, shape, logo, material, window placement, handles, proportions, or included accessories changed. Compare each generated screen against the original product image — if the product looks like a different model, color variant, or brand, fail the audit. The fix is to regenerate the affected screen with `generation_mode: "i2i"` and the original product image in `image_urls`.
